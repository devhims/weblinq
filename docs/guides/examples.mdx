...
export async function researchTopic(query) {
  ...
  return results.map((r, i) => ({ url: r.url, ...extracted[i].data.extracted }));
}
...
export async function monitor(url) {
  const shot = await post('/web/screenshot', { url, screenshotOptions: { fullPage: true } });

  const {
    data: { links },
  } = await post('/web/links', { url, visibleLinksOnly: true });
  const productLinks = links.filter((l) => /\/product\//.test(l.url)).slice(0, 10);
...
export async function buildStrategy(topic) {
  ...
  const {
    ...
    url: 'data:text/plain;base64,' + Buffer.from(JSON.stringify(analysis)).toString('base64'),
    responseType: 'text',
