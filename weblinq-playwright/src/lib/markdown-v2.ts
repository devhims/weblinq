 ...
 */
export async function markdownOperation(
  page: Page,
 ...
 */
export async function processContentToMarkdown(
  content: string,
  params: MarkdownParams,
): Promise<MarkdownResult> {
  try {
...
const processor = unified()
  ...
  .use(() => (tree: MdastRoot) => {
    ...
    visit<MdastRoot, 'paragraph'>(tree, 'paragraph', (node, idx, parent) => {
      ...
      if (
        next?.type === 'heading' &&
        toString(paragraph).trim() === toString(next).trim()
      ) {
    ...
    visit<MdastRoot, 'paragraph'>(tree, 'paragraph', (node, idx, parent) => {
      ...
      if (
        prev?.type === 'paragraph' &&
        toString(prev).trim() === toString(node).trim()
      ) {
        parent.children.splice(idx, 1);
      }
    });
    ...
    visit<MdastRoot, 'paragraph'>(tree, 'paragraph', (node) => {
      ...
      if (
        last.type === 'text' &&
        prev.type === 'link' &&
        last.value.trim().startsWith(prev.url)
      ) {
        node.children.pop();
