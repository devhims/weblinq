...
export async function pageGotoWithRetry(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.log(
        `✅ Page navigation successful on attempt ${attempt}/${maxAttempts} for ${url}`,
      );
      return;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err);
      console.warn(
        `❌ Page navigation attempt ${attempt}/${maxAttempts} failed for ${url}: ${errorMessage}`,
      );
      ...
      if (!isRetryableError || attempt === maxAttempts) {
        console.error(
          `🚫 Page navigation failed after ${attempt} attempts for ${url}: ${errorMessage}`,
        );
        throw err;
      }
