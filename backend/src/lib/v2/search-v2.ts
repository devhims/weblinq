...
abstract class BaseSearchHandler {
  ...
  async performSearch(page: Page, query: string, limit: number): Promise<InternalSearchResult> {
    ...
    await pageGotoWithRetry(page, url, { waitUntil: 'domcontentloaded', timeout: 10_000 });
...
class MultiEngineSearchHandler {
  ...
  private async runEnginesInParallel(
    ...
    const perEngineLimit = Math.max(3, Math.ceil(limit * 0.8));
    console.log(`💡 [OPTIMIZATION] Requesting ${perEngineLimit} results per engine for final limit of ${limit}`);

    // Get browser instance to create additional pages for parallel processing
    const browser = page.context().browser();
    if (!browser) {
      throw new Error('Browser instance not available for parallel page creation');
    }
    ...
    try {
      ...
      console.log(`🚀 [TIMING] Creating additional pages for ${engines.length} engines at ${pageCreationStartTime}ms`);
      ...
      console.log(
        `⚡ [TIMING] Parallel search completed in ${parallelTime}ms - engines ran concurrently for speed boost`,
      );
  ...
  private async runEnginesSequentially(
    page: Page,
    engines: SearchEngine[],
    query: string,
    limit: number,
    ...
    // Execute searches sequentially for simplicity and reliability in v2
    const sequentialSearchStartTime = Date.now();
    console.log(`🚀 [TIMING] Starting sequential search execution at ${sequentialSearchStartTime}ms`);
    ...
    for (const engine of engines) {
      ...
      try {
        const engineStartTime = Date.now();
        console.log(
          `🔍 [TIMING] Starting ${engine} search at ${engineStartTime}ms (requesting ${perEngineLimit} results)`,
        );

        // Add timeout to prevent any single engine from taking too long
        const searchPromise = handler.performSearch(page, query, perEngineLimit);
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error(`${engine} search timeout after 8 seconds`)), 8000),
        );
        ...
        console.log(`📊 ${engine}: Found ${filtered.length} results (${result.results.length} raw)`);
      } catch (error) {
        const errorTime = Date.now() - sequentialSearchStartTime;
        console.error(`❌ [TIMING] ${engine} search failed after ${errorTime}ms:`, error);
        engineResults[engine] = {
          error: String(error),
          success: false,
    ...
    const sequentialTime = Date.now() - sequentialSearchStartTime;
    console.log(`⚡ [TIMING] Sequential search completed in ${sequentialTime}ms`);
  ...
  async searchMultipleEngines(
    ...
    // Optimized tiered search strategy: Fast engines in parallel first, Startpage fallback only if needed
    const primaryEngines: SearchEngine[] = ['duckduckgo', 'bing'];
    const fallbackEngines: SearchEngine[] = ['startpage'];

    console.log(`🎯 [STRATEGY] Starting with fast engines in parallel: ${primaryEngines.join(', ')}`);

    try {
      // Tier 1: Run fast engines in parallel for significant performance boost
      ...
      if (uniqueResultsCount < targetThreshold) {
        ...
        const fallbackResults = await this.runEnginesSequentially(page, fallbackEngines, query, limit);
      ...
      // Emergency fallback: run primary engines sequentially (no Startpage unless absolutely needed)
      console.log(`🔄 [EMERGENCY] Falling back to sequential search with primary engines only`);
      const fallbackResults = await this.runEnginesSequentially(page, primaryEngines, query, limit);
...
/**
 * V2 Search operation for PlaywrightPoolDO - adapted from v1 multi-engine search
 * Now works with Playwright page instances managed by PlaywrightPoolDO
 */
export async function searchOperation(page: Page, params: SearchParams): Promise<SearchResult> {
  const overallStartTime = Date.now();
  console.log(`🔍 [TIMING] searchV2 started at ${overallStartTime}ms`);

  try {
    const handlerStartTime = Date.now();
    console.log(`🔍 [TIMING] Starting MultiEngineSearchHandler at ${handlerStartTime}ms`);
