...
export const githubCallback: AppRouteHandler<GithubCallbackRoute> = async (
  c,
) => {
  ...
  try {
    const baseUrl = c.env.BETTER_AUTH_URL || 'http://localhost:3000';
    const redirectUrl = `${baseUrl}/api/auth/callback/github?code=${code}&state=${
      state || ''
    }`;
...
export const signOut: AppRouteHandler<SignOutRoute> = async (c) => {
  const auth = c.get('auth');
  const { callbackURL } = await c.req
    .json()
    .catch(() => ({ callbackURL: undefined }));
