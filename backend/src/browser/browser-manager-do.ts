...
export class BrowserManagerDO extends DurableObject<CloudflareBindings> {
  private readonly MAX_BROWSER_DOS = 3; // Maximum total DOs (reduced from 10)
  ...
  async getAvailableBrowserDO(): Promise<{ id: string; sessionId: string }> {
    ...
    console.log(
      `BrowserManager: Retrieved ${browserDOs.length} DOs in ${
    ...
    if (availableDO) {
      ...
      console.log(
        `BrowserManager: Assigning idle DO: ${availableDO.id} with sessionId: ${
          availableDO.sessionId || 'pending'
        }`,
      );
      ...
      console.log(
        `BrowserManager: Assignment completed in ${
    ...
    // No idle DO available, create new one or wait for one to become available
    console.log(
      'BrowserManager: No idle DOs available, creating new DO or waiting',
    );
    ...
    console.log(
      `BrowserManager: Total operation time: ${Date.now() - operationStart}ms`,
    );

    // At max capacity, use promise-based queue instead of polling
    console.log(
      `BrowserManager: At max capacity (${browserDOs.length}/${this.MAX_BROWSER_DOS}), queueing request`,
    );
  ...
  private queueRequest(
    ...
    return new Promise<{ id: string; sessionId: string }>((resolve, reject) => {
      ...
      const timeoutId = setTimeout(() => {
        ...
        reject(
          new Error(
            `Request timeout after ${this.QUEUE_MAX_WAIT_TIME}ms - all DOs are busy`,
          ),
        );
      ...
      this.waitingRequests.set(requestId, {
        ...
        resolve: async (doId: string) => {
          ...
          if (!this.waitingRequests.has(requestId)) {
            console.log(
              `BrowserManager: Request ${requestId} already fulfilled, ignoring`,
            );
            return;
          }
          ...
          if (assignedDO) {
            console.log(
              `BrowserManager: Assigned queued request ${requestId} to DO: ${doId}`,
            );
            resolve({ id: assignedDO.id, sessionId: assignedDO.sessionId! });
          } else {
            reject(
              new Error(`DO ${doId} not found when fulfilling queued request`),
            );
          }
        },
        reject: (error: Error) => {
      ...
      console.log(
        `BrowserManager: Queued request ${requestId} (${this.waitingRequests.size} requests waiting)`,
      );
    });
  }
  ...
  async createBrowsersBatch(count: number): Promise<{
    ...
    for (let i = 0; i < actualCount; i++) {
      try {
        ...
        const doId = `browser-${Date.now()}-${Math.random()
          .toString(36)
          .substring(2, 15)}`;

        console.log(
          `BrowserManager: Creating browser DO ${
            i + 1
          }/${actualCount}: ${doId}`,
        );
        ...
        console.log(
          `BrowserManager: ✅ Created browser DO: ${doId} (${createdCount}/${actualCount})`,
        );

        // Add staggered delay between creations (except for the last one)
        if (i < actualCount - 1) {
          console.log(
            `BrowserManager: 🕒 Waiting ${this.BROWSER_CREATION_DELAY}ms before next creation...`,
          );
          await new Promise((resolve) =>
            setTimeout(resolve, this.BROWSER_CREATION_DELAY),
          );
        }
      } catch (error) {
    ...
    console.log(
      `BrowserManager: 🏁 Batch creation complete - created: ${createdCount}, skipped: ${skippedCount}`,
    );
  ...
  async updateSessionId(doId: string, sessionId: string): Promise<void> {
    console.log(
      `BrowserManager: Updating session ID for DO ${doId}: ${sessionId}`,
    );
    const browserDOs = await this.getBrowserDOs();
    ...
    if (browserDO) {
      console.log(
        `BrowserManager: Found DO ${doId}, updating session ID from ${
          browserDO.sessionId || 'none'
        } to ${sessionId}`,
      );
      ...
      console.log(
        `BrowserManager: ⚠️ DO ${doId} not found for session ID update, ignoring`,
      );
    }
  }
  ...
  async updateDOStatus(
    ...
    if (browserDO) {
      console.log(
        `BrowserManager: Found DO ${doId}, updating status from ${browserDO.status} to ${status}`,
      );
      ...
      if (status === 'error') {
        ...
        if (browserDO.errorMessage) {
          console.log(
            `BrowserManager: ✅ DO ${doId} recovered from error state`,
          );
          browserDO.errorMessage = undefined;
          // Keep errorCount for debugging purposes, but reset it periodically
        }
      }
      ...
      console.log(
        `BrowserManager: DO ${doId} not found in tracking, adding it (${
          browserDOs.length + 1
    ...
    if (status === 'idle' && this.waitingRequests.size > 0) {
      ...
      if (oldestRequest) {
        ...
        console.log(
          `BrowserManager: 🎯 Fulfilling queued request ${oldestRequest.id} with DO ${doId}`,
        );
  ...
  private async createNewBrowserDO(): Promise<BrowserDO | null> {
    ...
    try {
      ...
      await this.updateBrowserDO(newDO);
      console.log(
        `BrowserManager: Added new DO ${doId} to tracking with status: busy`,
      );
      console.log(
        `BrowserManager: 🔍 Expected BrowserDO to self-identify as: ${doId}`,
      );

      return newDO;
    } catch (error) {
      console.error(
        `BrowserManager: ❌ Failed to create new Browser DO:`,
        error,
      );
      return null;
    }
  ...
  private async getBrowserDOs(): Promise<BrowserDO[]> {
    const browserDOs =
      (await this.ctx.storage.get<Record<string, BrowserDO>>('browserDOs')) ||
      {};
    const browserDOArray = Object.values(browserDOs);
    console.log(
      `BrowserManager: Retrieved ${browserDOArray.length} DOs from storage`,
    );
  ...
  private async updateBrowserDO(browserDO: BrowserDO): Promise<void> {
    ...
    console.log(
      `BrowserManager: Stored DO ${browserDO.id} with status ${browserDO.status} to storage`,
    );
  }

  // Remove a Browser DO from tracking with proper cleanup
  private async removeBrowserDO(doId: string): Promise<void> {
    try {
      // First cleanup the BrowserDO properly
      console.log(
        `BrowserManager: 🧹 Cleaning up BrowserDO before removal: ${doId}`,
      );
      ...
      console.log(
        `BrowserManager: ✅ BrowserDO cleanup completed for: ${doId}`,
      );
    } catch (error) {
      console.error(
        `BrowserManager: ⚠️ Failed to cleanup BrowserDO ${doId}:`,
        error,
      );
    }
  ...
  async getStats(): Promise<{
    ...
    const dosWithInactivity = browserDOs.map((doStatus) => {
      ...
      const inactiveHours =
        Math.round((inactiveTime / (60 * 60 * 1000)) * 10) / 10;
    ...
    return {
      ...
      note:
        browserDOs.length === 0
          ? 'Development mode: DOs may reset between requests'
          : undefined,
    };
  }
  ...
  private async attemptRecovery(errorDO: BrowserDO): Promise<void> {
    ...
    try {
      console.log(
        `BrowserManager: 🛠️ Attempting background recovery of DO ${doId}`,
      );

      const browserDoRef = this.env.BROWSER_DO.idFromName(doId);
      const browserStub = this.env.BROWSER_DO.get(browserDoRef);

      const newSessionId: string | null = await browserStub.generateSessionId(
        doId,
      );

      if (newSessionId) {
        console.log(
          `BrowserManager: ✅ Recovery succeeded for DO ${doId}, new session: ${newSessionId}`,
        );
        ...
        if (this.waitingRequests.size > 0) {
          ...
          if (oldestRequest) {
            console.log(
              `BrowserManager: 🎯 Assigning recovered DO ${doId} to queued request ${oldestRequest.id}`,
            );
            oldestRequest.resolve(doId);
          }
        }
      ...
      console.log(
        `BrowserManager: ⚠️ Recovery attempt failed for DO ${doId}:`,
        err,
      );
    }
  }
  ...
  async getDoStatus(
    doId: string,
  ): Promise<'idle' | 'busy' | 'error' | 'unknown'> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (pathname === '/cleanup-do' && request.method === 'POST') {
        ...
        if (!doId) {
          return new Response(
            JSON.stringify({ error: 'Missing doId in request body' }),
            { status: 400 },
          );
        }
      ...
      if (pathname === '/delete-all' && request.method === 'POST') {
        try {
          console.log(
            'BrowserManager: 🧹 Starting complete cleanup of all BrowserDOs',
          );
          ...
          for (const doStatus of browserDOs) {
            try {
              console.log(
                `BrowserManager: 🧹 Cleaning up BrowserDO: ${doStatus.id}`,
              );
              ...
              console.error(
                `BrowserManager: ❌ Failed to cleanup BrowserDO ${doStatus.id}:`,
                error,
              );
              results.push({
                id: doStatus.id,
          ...
          console.log(
            'BrowserManager: ✅ Cleared all BrowserDO references from storage',
          );
          ...
          console.log(
            `BrowserManager: 🧹 Complete cleanup finished - ${successCount}/${totalDOs} successful`,
          );

          return new Response(JSON.stringify(response, null, 2));
        } catch (error) {
          console.error(
            'BrowserManager: ❌ Delete-all operation failed:',
            error,
          );
          return new Response(
            JSON.stringify({
      ...
      if (pathname === '/') {
        const endpoints = {
          ...
          endpoints: {
            'GET /stats':
              'Get current DO statistics (includes inactivity info and error DOs)',
            'POST /create-browsers':
              'Proactively create multiple browsers with staggered timing (optional: {"count": 3}) - NEW RECOMMENDED ENDPOINT',
            'POST /create-do':
              '⚠️ DEPRECATED: Create a new DO (use /create-browsers instead)',
            'POST /cleanup-do':
              'Clean up a specific BrowserDO (required: {"doId": "browser-xxx"})',
            'POST /delete-all':
              '⚠️  DESTRUCTIVE: Safely delete ALL BrowserDOs and clear all storage (clean slate)',
          },
          config: {
            inactiveCleanupHours: this.INACTIVE_CLEANUP_HOURS,
            cleanupCheckInterval: `${
              this.CLEANUP_CHECK_INTERVAL / (60 * 1000)
            } minutes`,
            browserCreationDelay: `${
              this.BROWSER_CREATION_DELAY / 1000
            } seconds`,
