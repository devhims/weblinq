...
import { createCustomerPortalSession, getSubscriptionStatus } from '@/lib/payments/actions';
...
function ManageSubscription() {
  ...
  const planStatus = hasActiveSubscription ? 'Active subscription' : 'Free plan';
        ...
        <div className="space-y-4">
            ...
            <div className="mb-4 sm:mb-0">
              ...
              {currentPlan === 'free' && (
                <p className="text-sm text-blue-600 mt-1">
                  Upgrade to Pro for 5x more credits (5,000/month) and monthly credit renewal
                </p>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t">
            <div>
              ...
              <p className="text-lg font-semibold text-green-600">{availableCredits.toLocaleString()}</p>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-700">Credits Used</p>
              <p className="text-lg font-semibold text-gray-600">{creditsUsed.toLocaleString()}</p>
            </div>
          </div>
          ...
          <div className="pt-2">
            <div className="flex justify-between text-sm text-gray-600 mb-1">
              ...
              <span>{Math.round((creditsUsed / totalCredits) * 100)}% used</span>
            </div>
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-800">
              💡 <strong>How credits work:</strong> Each web scraping task (scrape, screenshot, search, etc.) uses 1
              credit.
              {currentPlan === 'free'
          ...
          {availableCredits < 100 && currentPlan === 'free' && (
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                ⚠️ <strong>Running low on credits!</strong> You have {availableCredits} credits remaining. Consider
                upgrading to Pro for 5,000 monthly credits.
              </p>
            </div>
          )}
        </div>
